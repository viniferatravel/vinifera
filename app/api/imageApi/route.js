...
export async function GET(req) {
    try {
        ...
        return NextResponse.json(
            { success: true, results: imageUrls, resultPackage: imageUrlsForPackage },
            { status: 200 }
        );
    } catch (error) {
        return NextResponse.json(
...
// Handle JSON payload for specific actions
async function handleJsonPayload(payload) {
    try {
        switch (payload.action) {
            case "delete":
                if (!Array.isArray(payload.publicIds)) {
                    return NextResponse.json(
                        { success: false, error: "Invalid public IDs" },
                        { status: 400 }
                    );
                }

                await Promise.all(
                    payload.publicIds.map((id) =>
                        cloudinary.api.delete_resources(`vinifera/${payload.folder}/${id}`)
                    )
                );
                return NextResponse.json({ success: true }, { status: 200 });
            ...
            case "deletePlaces":
                if (!Array.isArray(payload.places)) {
                    return NextResponse.json(
                        { success: false, error: "Invalid places array" },
                        { status: 400 }
                    );
                }
                ...
                await Promise.all(
                    payload.places.map((place) =>
                        cloudinary.api.delete_resources_by_prefix(
                            `vinifera/${payload.folder}/places/${place}`
                        )
                    )
                );
            ...
            case "rename":
                const { oldFolderName, newFolderName } = payload;
